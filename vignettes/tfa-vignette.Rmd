...
A first step of analysis consists of adjusting the genotypic data for coverage. For those particular filtered genotypes, the impact of coverage bias on analyses is not substantial. In general, coverage may strongly bias the results of factor analyses, and this important to consider adjusting for it. To adjust for coverage, the `tfa` package has the function `coverage_adjust` which implements a correction based on a latent factor regression model. According to the PCA, there are $K = 3$ main axes of variation in the data. based on this result, three factors are used in the latent factor  regression model.
...
For running FA of the corrected data matrix (`geno`), the number of factors (`k`), and the drift parameter (`lambda`) must be specified. 

The number of factors could be determined by PCA or other methods for analysis of population structure. It could be taken equal to the number of axes in the PCA of the `geno` matrix. Since the `tfa` method is based low-rank approximation, a lower value such as `k = 2` can be accepted. The values `k = 2` may be useful to explore the relationships of samples with two pre-specified ancestral populations.  

The drift parameter (`lambda`) can be determined by a grid search procedure. Here, it was chosen as the value for which the effect of age was removed from the **third** factor of the `tfa` analysis performed below, and repeated several times.
...
The object `mod` contains `k` factors adjusted for temporal drift in allele frequency in its argument `mod$u`. The argument `mod$sing` contains the variance associated with each factor 

```{r}
 var_explained <- mod$singular.values[1:3]/(sum(mod$singular.values))
 var_explained
```
