...
The data set contains no missing genotypes. The missing values were imputed from a much larger database. This was done in a preliminary analysis by using a completion algorithm implemented in the R package `LEA`. We also centered both the rows and the columns of the genotype matrix.
...
For running FA of the adjusted data matrix (`geno`), the number of factors (`k`), and the drift parameter (`lambda`) must be specified. 

The **number of factors** could be determined by PCA or other methods for analysis of population structure. It could be taken equal to the number of axes specified in the previous coverage adjustement operation, or to the main axes of variation in a PCA of the `geno` matrix. Since the `tfa()` approach is based a low-rank approximation of the data matrix, a lower value such as `k = 2` could be accepted. For example, the value `k = 2` could be useful to explore relationships of samples when two ancestral populations are included in a dataset.
...
The object `mod$u` can be used to visualize the samples in the space defined by the two first axes. The result looks like a PC plot showing the relative positions of Great Britain samples with respect to their putative sources of ancestry.  
...
The result can be interpreted as the proportion of temporal variance explained by the k factors. The orange line is the value for which the effect of age is removed from the **second** factor of the `tfa` analysis.
