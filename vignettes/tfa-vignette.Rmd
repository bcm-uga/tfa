...
Ten thousands SNP genotypes were filtered for high-quality, relatively high coverage and low number of missing values. Individual metadata and sample ages are provided as separate objects. The command below shows how to load the data in the R environment and which groups have been included in the data set.
...
The data set contains no missing genotypes. The missing values were imputed from a much larger database. This was done in a preliminary analysis by using a completion algorithm implemented in the R package `LEA`. Running a standard PCA on the data matrix clearly shows there are three main axes of variation in the genotypic data. 
...
A first step of analysis consists of adjusting the genotypic data for coverage. For those particular filtered genotypes, the impact of coverage bias on analyses is not substantial. In general, coverage may strongly bias the results of factor analyses, and this important to consider adjusting for it. To adjust for coverage, the `tfa` package contains the function `coverage_adjust()` which implements a correction based on a latent factor regression model. According to the PCA result, four factors could be used in the latent factor regression model. 
...
For running FA of the corrected data matrix (`geno`), the number of factors (`k`), and the drift parameter (`lambda`) must be specified. The **number of factors** could be determined by PCA or other methods for analysis of population structure. It could be taken equal to the number of axes in the PCA of the `geno` matrix. Since the `tfa()` approach is based a low-rank approximation of the data matrix, a lower value such as `k = 3` could be accepted. The value `k = 3` could be useful to explore relationships of samples when three ancestral populations are specified. The **drift parameter** (`lambda`) can be determined by a grid search procedure (see `choose_lambda`). Here, this parameter was chosen as the value for which the effect of age was removed from the **third** factor of the `tfa` analysis performed below, which was repeated several times with different values of `lambda`.
...
The  object `mod` contains `k` factors adjusted for temporal drift in allele frequency. The results are contained in argument `mod$u`. The argument `mod$sing` contains the singular value (variance) associated with each factor. 

```{r}
 var_explained <- mod$singular.values[1:3]^2/(sum(mod$singular.values^2))
 round(var_explained, 3)
```

The object `mod$u` can be used to visualize the samples in their first axes. The result looks like a PC plot showing the relative positions of Great Britain samples with respect to their putative sources of ancestry.  
...
In addition to geometric representations, the R package `tfa` can provide estimates of ancestry proportions for a target population or for a list of target individuals given specified source population samples. The number of sources must be less or equal than the number of factors plus one. For this analysis, the metadata object must contain a column named **Instance.ID** indicating the ID of each individual and another column  named **Group.ID** indicating the group ID of each individual in the study.
...
The result can be interpreted as the proportion of temporal variance explained by each factor 1 and 2. The orange line is the value for the effect of age is removed from the **second** factor of the `tfa` analysis.
