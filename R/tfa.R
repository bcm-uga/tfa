...
#' points(mod$w[meta$Country == "Russia",],
#'       pch = 8, cex = .6, col = "darkblue")
#' points(mod$w[meta$Country == "Turkey",],
#'       pch = 8, cex = .6, col = "salmon3")
#' points(mod$w[meta$Group.ID == "England_MBA",],
#'       pch = 8, cex = .6, col = "yellow4")
#' abline(h = 0, lty = 2, col = "orange")
#'
#' @references \url{https://www.biorxiv.org/content/10.1101/801324v3}
#' François, O., Liégeois, S., Demaille, B., Jay, F. (2019). Inference of population genetic structure from temporal samples of DNA. bioRxiv, 801324.
#' @seealso \code{\link{BA_England}}
tfa <- function(sample_ages,
  ...
  # conversion des ages en temps dans (0,1)
  range_ages <- max(sample_ages) - min(sample_ages)
  # error corrected here
  tn <- 1 - (age_ancient - min(age_ancient))/range_ages

  # normalisation des temps par la variance des lignes
  # entre min et max de la variance
